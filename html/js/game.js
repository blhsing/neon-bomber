...
function throwGhostBomb(p){if(roundLocked||!p.ghost||p.alive||players.filter(q=>q.alive).length<2||p.ghostBomb&&!p.ghostBomb.dead)return false;let cells=ghostLandingCellsAt(p.ghostTrack,p.facing);if(!cells.length){if(p.type==='human')textPop(p.x,p.y-.35,'面向場內才可投彈','#ccecff');return false}let cell=cells[Math.floor(rand(cells.length))],source=makeBombSource(p,true),airborne={fromX:p.x,fromY:p.y,toX:cell.x,toY:cell.y,cells:cells.map(c=>({...c})),elapsed:0,duration:.48};let b={x:p.x,y:p.y,owner:p,source,range:p.range,fuse:GHOST_BOMB_FUSE,initialFuse:GHOST_BOMB_FUSE,dead:false,moving:null,airborne,passers:new Set(),mega:false,cluster:false,pierce:p.pierce,ghost:true,born:time};bombs.push(b);p.ghostBomb=b;p.stats.ghostThrows=(p.stats.ghostThrows||0)+1;sfx.kick();burst(p.x,p.y,p.color,18,2.4);return true}
...
function thinkGhostAI(p,dt){p.ghostThink-=dt;let living=players.filter(q=>q.alive);if(living.length<2)return{dx:0,dy:0,bomb:false};if(p.ghostThink<=0||!p.ghostAim||!p.ghostAim.target.alive){let level=GHOST_AI_TUNING[p.aiLevel]?p.aiLevel:'normal',cfg=AI_LEVELS[level],tuning=GHOST_AI_TUNING[level],old=p.ghostAim,candidates=[],seen=new Set();for(const target of living){let forecast=ghostForecastCells(target,tuning);for(const aim of forecast)for(const post of ghostFiringPosts(target,aim)){let k=`${target.id}:${post.track}:${post.facing.x},${post.facing.y}`;if(seen.has(k))continue;seen.add(k);let candidate=ghostPostScore(p,target,post,forecast,tuning);if(candidate)candidates.push(candidate)}}candidates.sort((a,b)=>b.rankScore-a.rankScore||a.distance-b.distance);let chosen=candidates[0]||null;if(chosen&&old?.target?.alive&&tuning.hold>0){let oldForecast=ghostForecastCells(old.target,tuning),current=ghostPostScore(p,old.target,old,oldForecast,tuning,false);if(current&&current.hitChance>0){let age=time-(old.chosenAt??time),oldDir=Math.sign(ghostTrackDelta(p.ghostTrack,current.track)),newDir=Math.sign(ghostTrackDelta(p.ghostTrack,chosen.track)),reversing=oldDir&&newDir&&oldDir!==newDir,margin=tuning.switchMargin+(reversing?tuning.reverseMargin:0);if(age<tuning.hold||chosen.baseScore<current.baseScore+margin)chosen=current}}if(chosen){chosen.chosenAt=sameGhostPost(old,chosen)?old.chosenAt??time:time;p.ghostAim=chosen}else p.ghostAim=null;p.ghostThink=rand(cfg.thinkMax*tuning.thinkScale,cfg.thinkMin*tuning.thinkScale)}if(!p.ghostAim){p.aiMode='幽靈待命';return{dx:0,dy:0,bomb:false}}let d=ghostTrackDelta(p.ghostTrack,p.ghostAim.track);if(Math.abs(d)>.16){p.aiMode='幽靈追蹤';let cur=ghostPoint(p.ghostTrack),next=ghostPoint(p.ghostTrack+Math.sign(d)*.08);return{dx:next.x-cur.x,dy:next.y-cur.y,bomb:false}}p.ghostTrack=wrapGhostTrack(p.ghostAim.track);let pos=ghostPoint(p.ghostTrack);p.x=pos.x;p.y=pos.y;p.facing={...p.ghostAim.facing};p.aiMode=p.ghostBomb&&!p.ghostBomb.dead?'幽靈等待爆破':'幽靈伏擊';return{dx:p.facing.x,dy:p.facing.y,bomb:!p.ghostBomb||p.ghostBomb.dead}}
...
function updatePlayers(dt){
 for(const p of players){p.inv=Math.max(0,p.inv-dt);p.stun=Math.max(0,p.stun-dt);p.frozen=Math.max(0,p.frozen-dt);p.reverse=Math.max(0,p.reverse-dt);p.slow=Math.max(0,p.slow-dt);p.aiBombCd=Math.max(0,p.aiBombCd-dt);if(p.dashing)p.dashing=Math.max(0,p.dashing-dt);if(!p.alive){if(!p.ghost)continue;let a=p.type==='human'?humanActions(p):thinkGhostAI(p,dt);moveGhost(p,a.dx,a.dy,dt);if(a.bomb)throwGhostBomb(p);continue}
   let a=p.type==='human'?humanActions(p):thinkAI(p,dt),dx=a.dx,dy=a.dy,bomb=a.bomb,skill=a.skill;
   if(p.reverse>0){dx=-dx;dy=-dy}movePlayer(p,dx,dy,dt);if(bomb)placeBomb(p);if(skill)useSkill(p);
